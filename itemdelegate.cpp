...
{
    if (index.row()!=4&&index.row()!=5) QStyledItemDelegate::paint(painter,option,index);
    else
    {
...
{
    if (index.row()!=4&&index.row()!=5) return QStyledItemDelegate::editorEvent(event,model,option,index);
...
QWidget *ItemDelegate::createEditor(QWidget *parent, const QStyleOptionViewItem &, const QModelIndex &index) const
{
    ...
    else if (index.row()==6) // Stack Size
    {
        QSpinBox*editor=new QSpinBox(parent);
        editor->setAlignment(Qt::AlignCenter);
        editor->setMinimum(-1);
        editor->setMaximum(2147483647);
        editor->setSingleStep(1048576);
        return editor;
    }
    else return NULL;
}
...
void ItemDelegate::setEditorData(QWidget *editor, const QModelIndex &index) const
{
    ...
    else if (index.row()==6) //Stack Size
    {
        ...
        bool ok;
        edit->setValue(index.model()->data(index,Qt::EditRole).toInt(&ok));
        if (!ok) edit->setValue(-1);
        return;
    }
}
