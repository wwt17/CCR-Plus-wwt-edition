...
void Problem::configureNew(const QString &typ, double timeLim, double memLim, const QString &check, bool opt, int stack)
{
    if (typ.size())
    {
        if (typ=="传统型") type=ProblemType::Traditional;   
    ...
    if (type==ProblemType::Traditional)
    {
        compilers.push_back(CompilerInfo(QString("gcc -o %1 %1.c -lm -static").arg(name)+(opt?" -O2":"")+(stack!=-1?QString(" -Wl,-stack=%1").arg(stack):""),QString("%1.c").arg(name)));
        compilers.push_back(CompilerInfo(QString("g++ -o %1 %1.cpp -lm -static").arg(name)+(opt?" -O2":"")+(stack!=-1?QString(" -Wl,-stack=%1").arg(stack):""),QString("%1.cpp").arg(name)));
        compilers.push_back(CompilerInfo(QString("fpc %1.pas").arg(name)+(opt?" -O2":"")+(stack!=-1?QString(" -Cs%1").arg(stack):""),QString("%1.pas").arg(name)));
    }
